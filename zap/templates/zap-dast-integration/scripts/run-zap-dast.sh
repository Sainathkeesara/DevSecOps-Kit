...
# Create temporary plan with substituted variables
PLAN_CONTENT=$(sed "s|{{TARGET_URL}}|$TARGET_URL|g; s|{{REPORT_DIR}}|/zap/wrk/reports|g" "$PLAN_FILE")
echo "$PLAN_CONTENT" > /tmp/zap-plan.yaml

docker run --rm --name "$CONTAINER_NAME" \
  -v /tmp/zap-plan.yaml:/zap/plan.yaml:ro \
  -v "$(pwd)/$OUTDIR":/zap/wrk/reports \
  "$ZAP_IMAGE" \
